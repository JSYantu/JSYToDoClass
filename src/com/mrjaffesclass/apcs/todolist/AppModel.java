...
public class AppModel implements MessageHandler {
  ...
  public void init() {
    ...
    messenger.subscribe("sortFirst", this);
    messenger.subscribe("sortLast", this);
  }
  ...
  public void messageHandler(String messageName, Object messagePayload) {
    ...
    switch (messageName) {
      ...
      // We've been told to sort all items from earliest to latest 
      // date. Do it, then send a confirmation message, then send the
      // updated to do list to others
      case "sortFirst":
        sortByDate(true);
        messenger.notify("saved", null, true);
      ...
      // We've been told to sort all items from latest to earliest 
      // date. Do it, then send a confirmation message, then send the
      // updated to do list to others
      case "sortLast":
        sortByDate(false);
   ...
   */
   public void sortByDate(boolean earliestFirst) {
      ...
      ToDoItem temp = new ToDoItem(-1, "If first date is null, then sort fails", true, new Date());
      toDoList.add(0,temp);
      do {
          notSorted = false;
